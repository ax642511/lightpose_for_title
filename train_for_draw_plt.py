"""
    ...
    本文件是已经写入了plt绘制损失的曲线图的train

    运行代码：
        python train.py --train-images-forlder ./coco/train2017/ --prepared-train-labels prepared_train_annotation.pkl --val-labels val_subset.json --val-images-folder ./coco/val2017/ --checkpoint-path checkpoint_iter_370000.pth checkpoint_iter_370000.pth --weight-only --num-refinement-stages 2
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    stride = 8  # 输入图像是特征图的倍数
    sigma = 7   # 生成关节点heatmap时，高斯核的标准差
    path_thickness = 1  # 生成paf时躯干的宽度
    ...
    for epochId in range(current_epoch, 280):
        #print("This is Epoch {}".format(str(epochId)))
        scheduler.step()
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(1, len(losses)):
                loss += losses[loss_idx] # 计算所有stage的loss
            loss /= batches_per_iter    # loss平均
            loss.backward()
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    ...
                    np.savetxt("./loss_data/stage1_pafs_loss.txt",stage1_pafs_loss)
                    np.savetxt("./loss_data/stage1_heatmaps_loss.txt",stage1_heatmaps_loss)
                    np.savetxt("./loss_data/stage2_pafs_loss.txt",stage2_pafs_loss)
                    np.savetxt("./loss_data/stage2_heatmaps_loss.txt",stage2_heatmaps_loss)
                    np.savetxt("./loss_data/stage3_pafs_loss.txt",stage3_pafs_loss)
                    np.savetxt("./loss_data/stage3_heatmaps_loss.txt",stage3_heatmaps_loss)
                    np.savetxt("./loss_data/total_loss.txt",total_array)

                # 从本地txt文件中读取数据
               
                stage1_pafs_loss = np.loadtxt('./loss_data/stage1_pafs_loss.txt') 
                stage1_heatmaps_loss = np.loadtxt('./loss_data/stage1_heatmaps.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-refinement-stages', type=int, default=2, help='number of refinement stages')
    ...
    parser.add_argument('--checkpoint-after', type=int, default=50,
